...
/* Сколько стеллажей физически влезет: площадь стеллажа плюс доля прохода. */
function racksFit(storageM2, widthCm) {
  const footprint = (widthCm / 100) * 0.4;      // м², сам стеллаж
  const withAisle = footprint * 2.1;            // проход и подход к полкам
  return Math.max(0, Math.floor(storageM2 / withAisle));
}
